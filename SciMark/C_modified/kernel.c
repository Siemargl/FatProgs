...
/* just original version, but resulted output in ms, comparable with python bench */
...
/// 50 is python times
        result = 50 * 1e3 * Stopwatch_read(Q) / cycles;
//printf("bench FFT(%d)=%g sec / %d cycles\n", N, Stopwatch_read(Q), cycles);
        Stopwatch_delete(Q);
        free(x);
        return result;
    }
...
/// 10 is python times
        result = 10* 1e3 * Stopwatch_read(Q) / cycles;
        Stopwatch_delete(Q);
...
/// 1e5 python times
        result = 1e5 * 1e3 * Stopwatch_read(Q) / cycles;
        Stopwatch_delete(Q);
        return result;
    }
...
/// 16 is python times
        result = 16 * 1e3 * Stopwatch_read(Q) / cycles;
...
///        result = LU_num_flops(N) * cycles / Stopwatch_read(Q) * 1.0e-6;
        result = 1e3 * Stopwatch_read(Q) / cycles;
